...
class TestAddressBackendFallback:
    ...
    def test_get_address_from_backends_validate(self, real_address):
        """Test get_address_from_backends with validate operation."""
        ...
        assert "backend_used" in result or "error" in result
        if "backend_used" in result:
            assert "is_valid" in result
            assert "confidence" in result

    def test_get_address_from_backends_geocode(self, real_address):
        """Test get_address_from_backends with geocode operation."""
        result = get_address_from_backends(
            MISSIVE_CONFIG_ADDRESS_BACKENDS,
            operation="geocode",
            **real_address,
    ...
    def test_get_address_from_backends_reverse_geocode(self):
        """Test get_address_from_backends with reverse_geocode operation."""
        result = get_address_from_backends(
            MISSIVE_CONFIG_ADDRESS_BACKENDS,
        ...
        assert "backend_used" in result or "error" in result
        if "backend_used" in result:
            assert "address_line1" in result or "formatted_address" in result

    def test_get_address_from_backends_invalid_operation(self):
        """Test get_address_from_backends with invalid operation."""
        result = get_address_from_backends(
            MISSIVE_CONFIG_ADDRESS_BACKENDS,
